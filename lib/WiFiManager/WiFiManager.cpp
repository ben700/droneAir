...
void WiFiManager::setupConfigPortal()
{
  ...
  server->serveStatic("/favicon.ico", LittleFS, favicon.c_str());
  server->serveStatic("/logo.jpeg", LittleFS, logo.c_str());
  server->serveStatic("/store_logo.png", LittleFS, store_logo.c_str());
  server->serveStatic("/qr.png", LittleFS, qr.c_str());

  server->serveStatic("/style.css", LittleFS, style.c_str());

  server->serveStatic("/wifi_lock_black_24dp.svg", LittleFS, wifi_lock_black_24dp.c_str());
  server->serveStatic("/lock_open_black_24dp.svg", LittleFS, lock_open_black_24dp.c_str());
  server->serveStatic("/wifi_black_24dp.svg", LittleFS, wifi_black_24dp.c_str());
  server->serveStatic("/keyboard_black_24dp.svg", LittleFS, keyboard_black_24dp.c_str());
  server->serveStatic("/save_black_24dp.svg", LittleFS, save_black_24dp.c_str());

  server->serveStatic("/variableFonts.js", LittleFS, variableFonts.c_str());
  server->serveStatic("/jquery-3.5.1.min.js", LittleFS, jquery.c_str());

  server->serveStatic("/Poppins-Regular.ttf", LittleFS, PoppinsRegular.c_str());
  server->serveStatic("/MerriweatherSans.ttf", LittleFS, MerriweatherSans.c_str());
...
{
  return autoConnect(_apName, NULL);
}
...
void WiFiManager::handleRoot()
{
  ...
  page += FPSTR(HTTP_END);
  page.replace("{v}", "Droneponics Device Network Configuration");
